...
def inject_custom_css():
    ...
    <style>
        ...
        .chat-bubble-me {
            background-color: #222; color: #fff; padding: 10px 16px; 
            border-radius: 18px; border-bottom-right-radius: 4px; 
            margin-bottom: 5px; float: right; clear: both; max-width: 85%;
        }
        .chat-bubble-other {
            background-color: #F2F2F2; color: #222; padding: 10px 16px; 
            border-radius: 18px; border-bottom-left-radius: 4px; 
            margin-bottom: 5px; float: left; clear: both; max-width: 85%;
        }
        .chat-bubble-ai {
        ...
        with col_node:
            for msg in chat_history_ai:
                if msg['role'] == 'user' and msg['content'] in nodes_map:
                    ...
                    with st.popover("●"):
                        score = node.get('m_score') if node.get('m_score') is not None else 0.5
        ...
        if prompt := st.chat_input("Input..."):
            ...
            with st.spinner(""):
                ...
                if analysis.get("valid", False):
                    ...
                    msc.save_node(st.session_state.username, prompt, analysis, "AI", vec)
