...
def analyze_meaning_background(text):
    # 🌟 严厉的过滤器：Explicitly filter out small talk
    prompt = f"""
    分析用户输入："{text}"
    
    【核心规则】
    1. 如果这句话是寒暄（如“吃了没”、“你好”、“晚安”）、简单陈述（如“下雪了”、“今天好热”）、或无意义的字符，必须返回 {{ "valid": false }}。
    2. 只有当包含：强烈的个人情绪、观点、反思、追问、或价值判断时，才返回 valid: true。
    
    返回 JSON:
    {{
        "valid": true/false,
        "care_point": "简短核心关切(5-8字)",
        "meaning_layer": "结构分析",
        "insight": "一句话洞察",
        "logic_score": 0.8, 
        "keywords": ["tag1"], 
...
def render_cyberpunk_map(nodes, height="250px", is_fullscreen=False):
    ...
    for i, node in enumerate(nodes):
        ...
        graph_nodes.append({"name": str(node['id']), "id": str(node['id']), "symbolSize": symbol_base * (0.8 + logic), "value": node['care_point'], "label": {"show": is_fullscreen, "formatter": node['care_point'][:5], "color": "#fff"}, "vector": vector, "keywords": keywords})
