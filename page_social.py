...
def render_friends_page(username, unread_counts):
    ...
    if node_count < 50 and not st.session_state.is_admin:
        ...
        with c2:
            ...
            # 2. 再注入 HTML (使用 f-string 安全注入变量)
            st.markdown(f"""
            <div class='lock-container'>
                <div class='lock-icon'>🔒</div>
                <div class='lock-title'>{i18n.get_text('lock_title')}</div>
                <div class='lock-quote'>{i18n.get_text('lock_msg')}</div>
                <div class='lock-stat-number'>{node_count} / 50</div>
    ...
    with col_chat:
        ...
        if partner:
            ...
            st.info(i18n.get_text('chat_sel'))### page__social.py ###
import streamlit as st
import streamlit_antd_components as sac
import msc_lib as msc
import msc_viz as viz
